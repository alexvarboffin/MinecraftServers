...
class ServersAdapter : ListAdapter<Any, BaseViewHolder<ViewBinding, Any>>(object :
    DiffUtil.ItemCallback<Any>(
    ) {
    override fun areItemsTheSame(oldItem: Any, newItem: Any): Boolean = when {
        oldItem is ServersDto && newItem is ServersDto -> oldItem.title == newItem.title
        else -> false
    }
    ...
    override fun onCreateViewHolder(
        ...
        else -> throw java.lang.IllegalArgumentException("ServersAdapter can't handle the $viewType type")
    }
...
private class ServersViewHolder(private val parent: ViewGroup) :
    ...
    override fun ListServerItemBinding.bind(value: ServersDto) {
        tittleServer.text = "Title:" + value.title
        tittleIp.text = "Ip" + value.ip
        tittlePort.text = "Port" + value.port
    }
}
