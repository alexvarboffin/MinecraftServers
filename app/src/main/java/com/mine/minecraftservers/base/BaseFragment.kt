...
abstract class BaseFragment<VIEW_BINDING : ViewBinding> : Fragment() {
    ...
    open fun setResultListenerWithCheckRequester(
        ...
        setFragmentResultListener(usedRequester) { provideRequestKey: String, bundle: Bundle ->
            clearFragmentResult(provideRequestKey)
            clearFragmentResultListener(provideRequestKey)
            handleResult(bundle)
        }
